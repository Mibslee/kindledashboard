...
Mac 控制，Kindle 显示。适合放在桌面上显示生活总览、天气、时间、中国双历、音乐、Codex 工作状态、Markdown 步骤文档和静态图片。
...
新增“生活”页面，把桌面上最常查看的时间、当月双历和天气放进同一个低打扰视图。它不是把已有天气页和日历页机械拼接，而是围绕“一眼知道现在、今天和接下来”重新取舍：

- 时间是第一视觉层级，横竖屏都能在约 1 米外快速读到。
- 当前天气使用大号温度、图标和天气状态；湿度、风和降雨风险保留为第二层。
- 月历逐日显示对应农历，节气当天优先显示节气名称。
- 今日农历、下一节气倒计时及准确日期独立显示，不需要进入日历页查找。
- 竖屏底部保留三个近期天气时点；横屏利用宽度并列五个时点。
- 横竖屏使用独立构图，不拉伸卡片，不用重复信息填补空白。

生活页不读取日程、不提供同步，也不申请 Calendar 或 Reminders 权限。
...
首页用于回答“现在最值得关注什么”。竖版保持时间、天气风险、当前 Codex 工作和设备状态的纵向顺序；横版把时间与天气放在左侧，把当前工作和 Mac 健康放在右侧，并补充雨势回落时间、农历、节气和年度进度。新增信息用于解释天气何时转好，不用无意义的大卡片填空。
...
- 左侧：时间、当前天气、最重要的降雨或出门建议，以及雨势何时回落。
...
生活页用于回答“现在几点、今天是什么日子、天气怎样、接下来会怎样”。竖版按时间、当前天气、今日农历、月历和短时天气向下阅读；横版将时间/天气/农历放在左侧，完整月历放在右侧，底部则用五列天气时间线建立连续性。
...
<table>
  ...
  <tr>
    <td><img src="docs/assets/previews/v0.4.0/portrait-life.png" alt="KindleDashboard 生活页竖屏"></td>
    <td><img src="docs/assets/previews/v0.4.0/landscape-life.png" alt="KindleDashboard 生活页横屏"></td>
  </tr>
</table>

横竖版信息取舍：

- 远距主读：时间、当前温度、天气状态和今日农历使用大号、高对比文字。
- 月历例外：日期与逐日农历允许使用更紧凑字号，但保持清楚的周列、今日高亮和节气加粗。
- 竖屏：只显示三个最近天气时点，避免底部信息过密。
- 横屏：显示五个天气时点，并把月份节气放进月历卡片的有效剩余空间。
- 今日农历和节气只出现一次；天气时间线补充未来变化，不重复当前天气卡。
- 所有传统历法计算都在本地完成，天气不可用时仍保留完整时间和双历能力。
...
天气页用于回答“现在是什么天气、接下来会怎样、要不要调整出行”。竖版按当前天气、风险提醒和逐时预报向下阅读；横版让当前天气和未来五个时间点同时可见，并在底部放置体感、湿度、风、气温范围以及行动建议的数据依据。
...
- 底部指标带补充体感、湿度、风与未来气温范围。
- 行动建议只出现一次；最下方改为显示峰值降雨和回落时间，解释建议从何而来。
- 天气不可用时显示稳定空状态，不让旧数据伪装成实时结果。

#### 8. 中国双历

日历页不再读取任何日程，也不需要同步。它用于理解当前时间、今天、公历月份、农历和季节节律。竖版适合依次读取日期与时间、月历、节气和年度进度；横版放大当前时间，把左侧拆成农历、下一个节气和年度进度，并使用月历下方空间展示本月节气与月份进度。
...
| 11 个竖版 SVG | 全部通过 XML 结构检查和视觉审阅 |
| 11 个横版 SVG | 全部通过 XML 结构检查和视觉审阅 |
